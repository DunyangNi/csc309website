...
function Settings({ close }) {
  ...
  const [birthday, setBirthday] = useState(data ? new Date(data?.birthday) : null)
  ...
  useEffect(() => {
    ...
    if (name && name !== data?.name) b.name = name
    if (email && email !== data?.email) b.email = email
    if (birthday && birthday !== data?.birthday)
      b.birthday = birthday.toISOString().substring(0, 10)
    if (avatarUrl && avatarUrl !== data?.avatarUrl) b.avatarUrl = avatarUrl
