...
function EventsPage() {
  ...
  const gridRows = events.map((event, index) => (
    ...
    >
      ...
      <div className="grid-cell">{new Date(event.startTime).toLocaleDateString()}</div>
      <div className="grid-cell">{new Date(event.endTime).toLocaleDateString()}</div>
      {/* 
      ...
      <div className="grid-cell"><DeleteEventButton eventId={event.id} setError={setError} currentPath={'/manager/events'} setEvents={setEvents}/></div>
