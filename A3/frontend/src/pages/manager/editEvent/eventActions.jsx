async function addOrganizer(eventId, utorid, setError, currentPath, navigate, setOrga) {
  try {
    const res = await fetch(`https://csc309website.onrender.com/events/${eventId}/organizers`, {
      method: 'POST',
      headers: {
...
async function removeOrganizer(eventId, userId, setError, currentPath, navigate, setOrga) {
  try {
    const res = await fetch(`https://csc309website.onrender.com/events/${eventId}/organizers/${userId}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('access_token')}`,
      },
...
async function addGuest(eventId, utorid, setError, currentPath, navigate, setGuests) {
  try {
    const res = await fetch(`https://csc309website.onrender.com/events/${eventId}/guests`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${localStorage.getItem('access_token')}`,
...
async function removeGuest(eventId, userId, setError, currentPath, navigate, setGuests) {
  try {
    const res = await fetch(`https://csc309website.onrender.com/events/${eventId}/guests/${userId}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('access_token')}`,
      },
